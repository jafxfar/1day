...
export default function Dashboard() {
  ...
  return (
    <Layout>
      <div className="px-2 pt-3 pb-1 space-y-[16px]">
        ...
        <div className="flex items-start justify-between">
          ...
          <button className="relative p-[8px] bg-card rounded-xl border border-border hover:bg-accent transition-colors" onClick={() => navigate('/profile')}>
            <Bell className="w-1.8 h-1.8 text-muted-foreground" />
            <span className="absolute top-[4px] right-[4px] w-[8px] h-[8px] bg-primary rounded-full" />
          </button>
        </div>
        ...
        {!hasCompletedMorning && !isLoadingCheckins && (
          <button
            ...
            className="w-full flex items-center justify-between bg-primary text-primary-foreground rounded-2xl px-1.5 py-1 hover:opacity-90 transition-opacity"
          >
            <div className="flex items-center gap-1">
              <Sun className="w-1.8 h-1.8" />
            ...
            <ChevronRight className="w-1.5 h-1.5 opacity-70" />
          </button>
        )}
          ...
          <>
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <div key={label} className="bg-card border border-border rounded-2xl p-1 text-center">
                  <Icon className="w-1.8 h-1.8 text-muted-foreground mx-auto mb-1" />
                  <p className="text-xl font-bold text-foreground leading-none">{value}</p>
                  <p className="text-[10px] text-muted-foreground mt-[8px]">{label}</p>
                </div>
              ))}
            </div>
            <div className="bg-card border border-border rounded-2xl px-1 py-1">
              <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-0.5">Today's Focus 🎯</p>
        ...
        <div className="grid grid-cols-2 gap-[8px]">
          <button onClick={() => navigate('/habits')}
            className="bg-card border border-border rounded-2xl p-1.5 text-left hover:border-primary/40 transition-colors">
            ...
            <Progress value={habits.length ? (completedHabits / habits.length) * 100 : 0} className="mt-1 h-0.5" />
          </button>
          <button onClick={() => navigate('/goals')}
            ...
            <Progress value={avgGoalProgress} className="mt-1 h-0.5" />
          </button>
        </div>

        {/* ── AI Insight ── */}
        <button onClick={() => navigate('/ai')}
          className="w-full bg-card border border-border rounded-2xl p-1 text-left hover:border-primary/40 transition-colors">
          <div className="flex items-start gap-1">
            <div className="w-3 h-3 rounded-xl bg-primary/10 flex items-center justify-center shrink-0">
              <Bot className="w-1.8 h-1.8 text-primary" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-0.5">
                <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">AI Coach</span>
                <ChevronRight className="w-1.5 h-1.5 text-muted-foreground" />
              </div>
        ...
        <div className="grid grid-cols-2 gap-[8px]">
          <button onClick={() => navigate('/calendar')}
            className="bg-card border border-border rounded-2xl px-1 py-1 text-left hover:border-primary/40 transition-colors">
            <p className="text-lg">📅</p>
            <p className="text-sm font-medium text-foreground mt-[3px]">Calendar</p>
            <p className="text-xs text-muted-foreground">View your timeline</p>
          </button>
          <button onClick={() => navigate('/evening')}
            className="bg-card border border-border rounded-2xl px-1 py-1 text-left hover:border-primary/40 transition-colors">
            <Moon className="w-1.8 h-1.8 text-muted-foreground" />
            <p className="text-sm font-medium text-foreground mt-[3px]">Evening</p>
